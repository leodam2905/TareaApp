...
const STATUS_COLOR: Record<string, string> = { OPEN: C.sky, ASSIGNED: C.emerald, CLOSED: C.slate400 };
  ...
  const renderItem = ({ item: req }: { item: JobRequest }) => (
    <View style={s.card}>
      <View style={s.cardTop}>
        ...
        <View style={[s.badge, { backgroundColor: (STATUS_COLOR[req.status] ?? C.slate400) + "22" }]}>
          <Text style={[s.badgeTxt, { color: STATUS_COLOR[req.status] ?? C.slate400 }]}>{req.status}</Text>
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  h1:          { color: C.white, fontSize: 24, fontWeight: "900" },
  card:        { backgroundColor: "#1E293B", borderRadius: 16, padding: 16, marginBottom: 14, borderWidth: 1, borderColor: "rgba(255,255,255,0.06)" },
  ...
  appsHeader:  { color: C.slate300, fontSize: 12, fontWeight: "700", marginTop: 6, marginBottom: 4 },
  emptyApps:   { color: C.slate500, fontSize: 13, fontStyle: "italic", paddingVertical: 6 },
  emptyBig:    { color: C.slate400, textAlign: "center", marginTop: 64, fontSize: 15 },
  appRow:      { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 10, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.05)" },
