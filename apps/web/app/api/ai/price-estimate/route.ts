...
export async function POST(req: NextRequest) {
  ...
  const budget = await reserveAiBudget("price-estimate");
  if (!budget.ok) {
    console.warn(JSON.stringify({ kind: "ai_budget_exceeded", route: "price-estimate", used: budget.used, cap: budget.cap }));
